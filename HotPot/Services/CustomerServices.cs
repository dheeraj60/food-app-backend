...
namespace HotPot.Services
{
    public class CustomerServices : ICustomerServices
    {
        ...
        public async Task<LoginUserDTO> LogIn(LoginUserDTO loginCustomer)
        {
            ...
            // Check if the repository returned valid data
        ...
        {
            var user = await _userRepo.GetAsync(resetPassword.UserName);
            if (user == null)
                throw new InvalidUserException();

            // Generate new password hash
            var newPasswordHash = getEncryptedPassword(resetPassword.NewPassword, user.Key);
